...
# Lazy import for canonical.bicep_state - only loaded when emit_bicep_v2_run is called
_bicep_state = None

def _get_bicep_state():
    """Lazy import canonical.bicep_state to avoid crash if module not installed."""
    global _bicep_state
    if _bicep_state is None:
        try:
            from canonical import bicep_state as bs
            _bicep_state = bs
        except ImportError as e:
            raise ImportError(
                "canonical.bicep_state not available. "
                "This module is required for bicep_v2 proof generation. "
                "Install with: pip install capseal[bicep]"
            ) from e
    return _bicep_state
...
def _manifest_dict(
    rng,  # AddressableRNG - type checked at runtime
    num_paths: int,
    num_channels: int,
    audit_k: int,
    precision_bits: int,
...
def emit_bicep_v2_run(
    ...
    # Lazy import - only load bicep_state when this function is called
    bs = _get_bicep_state()
    ...
    rng = bs.AddressableRNG.from_hex(seed_hex)
    n_leaves = bs.total_leaves(num_paths, num_channels)

    manifest = _manifest_dict(rng, num_paths, num_channels, audit_k, precision_bits, sde_params)
    manifest_hash = hashlib.sha256(bs.canonical_json_bytes(manifest)).hexdigest()

    # Genesis state (quantized RNG)
    state_q = [bs.quantize(rng.rand("state_init", 0, idx), precision_bits) for idx in range(n_leaves)]
    state_root, levels = bs.build_merkle_tree(state_q)
    ...
    with open(trace_path, "w") as tf:
        for t in range(num_steps):
            ...
            x_t_q = [bs.quantize(rng.rand("input", t, i), precision_bits) for i in range(num_channels)]
            ...
            rng_use_hash = hashlib.sha256(bs.canonical_json_bytes(rand_addrs)).hexdigest()
            ...
            for idx in range(n_leaves):
                ...
                if jump_rate > 0:
                    ...
                    next_q = bs.sde_step_with_jump(
                        state_q[idx], sde_params, eps, jump_flag, jump_mag, precision_bits
                    )
                else:
                    next_q = bs.sde_step_em(state_q[idx], sde_params, eps, precision_bits)
                state_post_q.append(next_q)

            state_q = state_post_q
            state_root_new, levels_new = bs.build_merkle_tree(state_q)

            # Update output_chain
            output_chain_new = hashlib.sha256(
                output_chain.encode() + bs.canonical_json_bytes(x_t_q)
            ).hexdigest()
            ...
            challenge_seed = bs.compute_challenge_seed(manifest_hash, head, state_root, rng_use_hash)
            audit_indices = bs.sample_audit_indices(challenge_seed, n_leaves, audit_k)
            audit = bs.generate_audit_opening(
                t,
                state_pre_q,
                state_post_q,
                levels_pre,
            ...
            row_digest = hashlib.sha256(bs.canonical_json_bytes(row)).hexdigest()
