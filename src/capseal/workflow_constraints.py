"""AIR constraints for WorkflowAIR.

The FRI proof enforces these constraints to prove workflow DAG execution integrity.

Constraint Types:
- Transition constraints: Between consecutive rows (consecutive DAG nodes in topo order)
...
Transition Constraints:
  1. Chain constraint: Row[i].receipt_hash == Row[i+1].prev_receipt_hash
     Proves the DAG chain is unbroken.
     Encoded as: row_next[9] - row_curr[11] == 0 (lo)
                 row_next[10] - row_curr[12] == 0 (hi)

  2. Ordering constraint: Row[i+1].node_index == Row[i].node_index + 1
     Proves nodes are in topological order.
     Encoded as: row_next[0] - row_curr[0] - 1 == 0

Boundary Constraints:
  1. First row: row[9] == 0 AND row[10] == 0 (no previous receipt for root node)
  2. Last row: row[11] and row[12] must equal the publicly declared dag_root_lo/hi

Per-Row Constraints:
  1. Status flags consistency: if bit0 == 0 (fail), output_hash must still be present
     (we prove failures too, not just successes)
  2. All field elements are < GOLDILOCKS_P (range check)
"""
...
from capseal.workflow_air import (
    GOLDILOCKS_P,
...
class WorkflowConstraints:
    """Container for workflow AIR constraint definitions.
    ...
    CONSTRAINT_NAMES = [
        ...
        "ordering",           # topological order
        "first_prev_lo",      # first row prev_receipt is 0 (lo)
        "first_prev_hi",      # first row prev_receipt is 0 (hi)
        "last_receipt_lo",    # last row receipt matches declared root (lo)
        "last_receipt_hi",    # last row receipt matches declared root (hi)
    ]
    ...
    @staticmethod
    def eval_boundary_last_receipt_lo(row_last: list[int], dag_root_lo: int) -> int:
        """Evaluate last row constraint: row[11] == dag_root_lo.

        Returns 0 if constraint is satisfied, non-zero otherwise.
        """
        return (row_last[11] - dag_root_lo) % GOLDILOCKS_P

    @staticmethod
    def eval_boundary_last_receipt_hi(row_last: list[int], dag_root_hi: int) -> int:
        """Evaluate last row constraint: row[12] == dag_root_hi.

        Returns 0 if constraint is satisfied, non-zero otherwise.
        """
        return (row_last[12] - dag_root_hi) % GOLDILOCKS_P
...
def verify_workflow_trace(
    rows: list[list[int]],
    dag_root_hash: str,
) -> tuple[bool, list[ConstraintResult]]:
    """Verify that a workflow trace satisfies all AIR constraints.

    This is the verification logic that the FRI proof proves.

    Args:
        rows: List of 14-element rows (trace matrix)
        dag_root_hash: SHA256 hex hash that the last row's receipt must match
    ...
    for i, row in enumerate(rows):
        if len(row) != WORKFLOW_AIR_ROW_WIDTH:
            results.append(ConstraintResult(
                name=f"row_width_{i}",
                satisfied=False,
                expected=WORKFLOW_AIR_ROW_WIDTH,
                actual=len(row),
    ...
    for i, row in enumerate(rows):
        if not WorkflowConstraints.check_field_bounds(row):
            results.append(ConstraintResult(
    ...
    first_prev_lo = WorkflowConstraints.eval_boundary_first_prev_lo(rows[0])
    results.append(ConstraintResult(
        name="first_prev_lo",
        satisfied=(first_prev_lo == 0),
        expected=0,
        actual=first_prev_lo,
        message="First row prev_receipt_lo must be 0",
    ))

    first_prev_hi = WorkflowConstraints.eval_boundary_first_prev_hi(rows[0])
    results.append(ConstraintResult(
        name="first_prev_hi",
        satisfied=(first_prev_hi == 0),
        expected=0,
        actual=first_prev_hi,
        message="First row prev_receipt_hi must be 0",
    ))

    # Boundary constraint: last row receipt matches dag_root
    dag_root_lo, dag_root_hi = sha256_to_field_pair(dag_root_hash)

    last_receipt_lo = WorkflowConstraints.eval_boundary_last_receipt_lo(rows[-1], dag_root_lo)
    results.append(ConstraintResult(
        name="last_receipt_lo",
        satisfied=(last_receipt_lo == 0),
        expected=dag_root_lo,
        actual=rows[-1][11],
        message="Last row receipt_hash_lo must match dag_root_lo",
    ))

    last_receipt_hi = WorkflowConstraints.eval_boundary_last_receipt_hi(rows[-1], dag_root_hi)
    results.append(ConstraintResult(
        name="last_receipt_hi",
        satisfied=(last_receipt_hi == 0),
        expected=dag_root_hi,
        actual=rows[-1][12],
        message="Last row receipt_hash_hi must match dag_root_hi",
    ))
    ...
    for i in range(len(rows) - 1):
        ...
        # Chain constraint (lo)
        chain_lo = WorkflowConstraints.eval_transition_chain_lo(row_curr, row_next)
        results.append(ConstraintResult(
            name=f"chain_lo_{i}",
        ...
        # Chain constraint (hi)
        chain_hi = WorkflowConstraints.eval_transition_chain_hi(row_curr, row_next)
        results.append(ConstraintResult(
            name=f"chain_hi_{i}",
        ...
        # Ordering constraint
        ordering = WorkflowConstraints.eval_transition_ordering(row_curr, row_next)
        results.append(ConstraintResult(
            name=f"ordering_{i}",
            satisfied=(ordering == 0),
            expected=row_curr[0] + 1,
            actual=row_next[0],
            message=f"Row {i+1} node_index must be row {i} node_index + 1",
        ))
...
def build_composition_vector(
    ...
    Args:
        ...
        dag_root_lo: Expected final receipt hash (lo)
        dag_root_hi: Expected final receipt hash (hi)
    ...
    """
    if alphas is None:
        alphas = {name: 1 for name in WorkflowConstraints.CONSTRAINT_NAMES}
    ...
    for i in range(num_rows):
        ...
        if i == 0:
            first_prev_lo = WorkflowConstraints.eval_boundary_first_prev_lo(row)
            first_prev_hi = WorkflowConstraints.eval_boundary_first_prev_hi(row)
        ...
        if i == num_rows - 1:
            last_receipt_lo = WorkflowConstraints.eval_boundary_last_receipt_lo(row, dag_root_lo)
            last_receipt_hi = WorkflowConstraints.eval_boundary_last_receipt_hi(row, dag_root_hi)
        ...
        if i < num_rows - 1:
            row_next = rows[i + 1]
            chain_lo = WorkflowConstraints.eval_transition_chain_lo(row, row_next)
            chain_hi = WorkflowConstraints.eval_transition_chain_hi(row, row_next)
            ordering = WorkflowConstraints.eval_transition_ordering(row, row_next)
...
def derive_constraint_alphas(seed: bytes) -> dict[str, int]:
    ...
    alphas: dict[str, int] = {}
    for i, name in enumerate(WorkflowConstraints.CONSTRAINT_NAMES):
